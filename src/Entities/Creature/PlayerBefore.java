...
public class PlayerBefore extends Creature{

	// ANIMATIONS
		private Animation animLeft , animRight , animRunLeft , animRunRight;
		
		public PlayerBefore(Handler handler, float x, float y, float speed) {
			...
			//ANIMATIONS INSTANCE
			animLeft = new Animation(120, Asset.player_before_left);
			animRight = new Animation(120, Asset.player_before_right);
			animRunLeft = new Animation(100 , Asset.player_before_run_left);
			animRunRight = new Animation(100, Asset.player_before_run_right);
		}
		...
		private BufferedImage getCurrentFrame() {
			if(!handler.getKeyManager().run) {
				if(xMove < 0) {
					...
					animLeft.resetIndex();
					return Asset.player_before_idle;
				}
			} else  {
				if(xMove < 0) {
